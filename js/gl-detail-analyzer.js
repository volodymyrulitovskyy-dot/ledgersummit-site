(function () {
  ...
  const FLAG_LABELS = {
    ...
    unusual_amount: 'Unusual amount vs account pattern',
    missing_description: 'Missing or generic description',
    weekend_posting: 'Weekend posting',
    round_dollar: 'Large round-dollar entry',
  ...
  function renderFlagSummary(flagCounts) {
    ...
    if (!topFlags.length) {
      return '<p class="glda-status muted">No anomaly groups were detected with the current mapping.</p>';
    }

    return topFlags.map(([flag, count]) => {
      const ratio = total ? (count / total) * 100 : 0;
      return `
        <div class="glda-flag-item">
          <strong>${escapeHtml(formatNumber(count))}</strong>
          <span>${escapeHtml(FLAG_LABELS[flag] || flag)}</span>
          <div class="glda-flag-bar"><span style="width:${Math.max(6, ratio)}%"></span></div>
        </div>
      `;
    }).join('');
  }
  ...
  function renderFindingsTable(flaggedRows) {
    if (!flaggedRows.length) {
      return '<p class="glda-status muted">No rows were flagged. Try remapping the file or review a different export.</p>';
    }

    return `
      <div class="glda-table-wrap">
        <table class="glda-table">
          <thead>
            <tr>
              <th>Risk</th>
              <th>Date</th>
          ...
          <tbody>
            ${flaggedRows.slice(0, 25).map((row) => `
              <tr>
                <td><span class="glda-score">${escapeHtml(String(row.riskScore))}</span></td>
                ...
                <td>
                  <div class="glda-flag-tags">
                    ${row.flags.map((flag) => `<span class="glda-flag-tag">${escapeHtml(FLAG_LABELS[flag] || flag)}</span>`).join('')}
                  </div>
                </td>
              </tr>
            `).join('')}
  ...
  function renderHotspots(hotspots) {
    if (!hotspots.length) {
      return '<p class="glda-status muted">Hot spots will appear here after the analyzer processes enough account activity.</p>';
    }

    return hotspots.map((hotspot) => `
      <article class="glda-hotspot-card">
        <h4>${escapeHtml(hotspot.account)}</h4>
        <p>High-review account based on flagged volume, large-value movement, and manual activity.</p>
        <div class="glda-hotspot-stats">
          <div class="glda-hotspot-stat">
  ...
  function renderJournalInsights(journalInsights) {
    if (!journalInsights.length) {
      return '<p class="glda-status muted">Map a journal ID column to surface journal-level balancing insights.</p>';
    }

    return journalInsights.map((journal) => `
      <article class="glda-insight-card">
  ...
  function renderExplorer(rows, term) {
    ...
    if (!filtered.length) {
      return '<p class="glda-status muted">No rows matched that search.</p>';
    }
          ...
          <tbody>
            ${filtered.map((row) => `
              <tr>
                ...
                <td>${row.flags.length ? `<div class="glda-flag-tags">${row.flags.map((flag) => `<span class="glda-flag-tag">${escapeHtml(FLAG_LABELS[flag] || flag)}</span>`).join('')}</div>` : '<span class="glda-status muted">Clear</span>'}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
  ...
  function updateSummaryCards(metrics) {
    ...
    document.querySelector('[data-metric="rows"]').innerHTML = `
      ...
      <span>rows analyzed from the current export.</span>
      <div class="glda-meter"><span style="width:100%"></span></div>
    `;
    document.querySelector('[data-metric="flagged"]').innerHTML = `
      <strong>${escapeHtml(formatNumber(metrics.flaggedRows))}</strong>
      <span>rows surfaced for human review first.</span>
      <div class="glda-meter"><span style="width:${Math.max(6, flaggedRatio * 100)}%"></span></div>
    `;
    document.querySelector('[data-metric="accounts"]').innerHTML = `
      <strong>${escapeHtml(formatNumber(metrics.accounts))}</strong>
      <span>accounts represented in the file.</span>
      <div class="glda-meter"><span style="width:${Math.min(100, metrics.accounts * 3)}%"></span></div>
    `;
    document.querySelector('[data-metric="net"]').innerHTML = `
      <strong>${escapeHtml(formatMoney(metrics.netBalance))}</strong>
      <span>net balance across loaded rows.</span>
      <div class="glda-meter"><span style="width:${Math.min(100, metrics.totalAbsoluteValue ? Math.abs(metrics.netBalance) / metrics.totalAbsoluteValue * 100 : 0)}%"></span></div>
    `;
  }

  function setStatus(message, type) {
    const status = document.getElementById('glda-status');
    status.className = `glda-status${type ? ` ${type}` : ''}`;
  ...
  function handleParsedData(parsed, autoAnalyze) {
    ...
    const mappingWrap = document.getElementById('glda-mapping-grid');
    mappingWrap.innerHTML = createMappingMarkup(parsed.headers);
    updateMappingInputs(state.mapping);

    document.getElementById('glda-setup-meta').innerHTML = `
      <span class="glda-meta-chip"><strong>${escapeHtml(formatNumber(parsed.rows.length))}</strong> rows loaded</span>
      <span class="glda-meta-chip"><strong>${escapeHtml(formatNumber(parsed.headers.length))}</strong> columns detected</span>
      <span class="glda-meta-chip"><strong>${escapeHtml(parsed.delimiter === '\t' ? 'Tab-delimited' : parsed.delimiter)}</strong> delimiter</span>
    `;
  ...
  function runAnalysis() {
    try {
      ...
      document.getElementById('glda-flag-grid').innerHTML = renderFlagSummary(analysis.flagCounts);
      document.getElementById('glda-findings-table').innerHTML = renderFindingsTable(analysis.flaggedRows);
      document.getElementById('glda-hotspot-grid').innerHTML = renderHotspots(analysis.hotspots);
      document.getElementById('glda-insight-grid').innerHTML = renderJournalInsights(analysis.journalInsights);
      document.getElementById('glda-explorer-table').innerHTML = renderExplorer(analysis.rows, '');
      document.getElementById('glda-search').value = '';
      document.getElementById('glda-results').hidden = false;
      document.getElementById('glda-empty').hidden = true;
      document.getElementById('glda-export').disabled = analysis.flaggedRows.length === 0;

      const flaggedRatio = analysis.metrics.rows ? analysis.metrics.flaggedRows / analysis.metrics.rows : 0;
      setStatus(`Analysis complete. ${analysis.metrics.flaggedRows} of ${analysis.metrics.rows} rows were prioritized for review (${formatPercent(flaggedRatio)}).`, 'success');
    } catch (error) {
  ...
  function bindEvents() {
    const fileInput = document.getElementById('glda-file');
    const textarea = document.getElementById('glda-paste');
    const analyzeButton = document.getElementById('glda-analyze');
    const sampleButton = document.getElementById('glda-sample');
    const pasteButton = document.getElementById('glda-paste-button');
    const resetButton = document.getElementById('glda-reset');
    const exportButton = document.getElementById('glda-export');
    const searchInput = document.getElementById('glda-search');
    ...
    analyzeButton.addEventListener('click', () => {
      if (!state.parsed) {
        setStatus('Load a file or paste data before running the analyzer.', 'error');
        return;
      }
      runAnalysis();
    });
    ...
    resetButton.addEventListener('click', () => {
      ...
      textarea.value = '';
      fileInput.value = '';
      document.getElementById('glda-mapping-grid').innerHTML = '';
      document.getElementById('glda-setup-meta').innerHTML = '';
      document.getElementById('glda-results').hidden = true;
      document.getElementById('glda-empty').hidden = false;
      document.getElementById('glda-export').disabled = true;
      document.getElementById('glda-search').value = '';
      setStatus('Tool reset. Load a new export or try the sample ledger.', 'muted');
    });
    ...
    searchInput.addEventListener('input', () => {
      ...
      document.getElementById('glda-explorer-table').innerHTML = renderExplorer(state.analysis.rows, searchInput.value);
    });
  }

  function init() {
    const root = document.getElementById('glda-app');
    if (!root) {
      return;
    }

    bindEvents();
    setStatus('Load a CSV export or try the sample ledger to start.', 'muted');
  }
